...
def hierarchical_clustering(X, n_clusters=3):
    # 使用ward方法计算层次聚类（最小化每次合并的方差）
    Z = linkage(X, method='ward')
    # 将聚类树切割为指定数量的簇（n_clusters）
...
plt.subplot(1, 2, 1)
dendrogram(Z)
plt.title('Hierarchical Clustering Dendrogram')  # 树状图标题
...
# ----- 6. 聚类结果输出 -----
# 输出层次聚类标签
print("Hierarchical Clustering Labels:", labels_hierarchical)  # 打印每个通道的聚类标签
